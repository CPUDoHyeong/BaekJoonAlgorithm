...
public class Q2675 {
	...
	public static void main(String[] args) {
		...
		int inputLoop = sc.nextInt();
		int[] loopCount = new int[inputLoop];
		String[] inputArr = new String[inputLoop];
		...
		for(int i = 0; i < inputLoop; i++) {
			for(int j = 0; j < inputArr[i].length(); j++) {
				for(int x = 0; x < loopCount[i]; x++) {
					System.out.print(inputArr[i].charAt(j));
				}
